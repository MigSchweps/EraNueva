...
function mh_themes_setup() {
	...
	add_image_size('content', 700, 270, true);
...
if (!function_exists('mh_head')) {
	function mh_head() {
		global $options;
		echo '<link rel="stylesheet" href="http://fonts.googleapis.com/css?family=Open+Sans">' . "\n";	
		echo '<link rel="stylesheet" href="' . get_bloginfo('stylesheet_url') . '" media="screen">' . "\n";
		echo '<link rel="pingback" href="' . get_bloginfo('pingback_url') . '"/>' . "\n";
	}
}
add_action('wp_head', 'mh_head', 1);
...
if (!function_exists('mh_page_title_output')) {
	function mh_page_title_output() {
		if (!is_front_page()) { ?>
			...
			<h1 class="page-title"><?php mh_page_title(); ?></h1> <?php
		}
	}
}
...
if (!function_exists('custom_image_size_choose')) {     
	function custom_image_size_choose($sizes) {
		global $options;
		if (isset($options['site_width']) && $options['site_width'] == 'large' && isset($options['2nd_sidebar']) && !$options['2nd_sidebar']) {
